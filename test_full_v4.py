...
只输出打标后的文本，完整保留所有[数字]索引行！"""
    ...
    system_prompt = "你是标签生成器，只输出打标后的内容，完整保留所有[数字]索引行。"
...
【任务】这是动态列表（如人员表、业绩表），请：
1. 仅保留表头行和第一个数据行
2. 删除其他所有空白数据行
3. 在第一个数据行开头添加循环语法：{{{{tr for item in 列表名称}}}}
4. 单元格使用泛型标签：{{{{item.字段名}}}}
5. 列表名称根据上下文命名（如"项目负责人列表"、"业绩列表"）

【输出】只输出打标后的表格文本"""
    else:
        prompt = f"""【角色】标书静态表单打标专家
...
for batch_start in range(0, len(valid_paragraphs), batch_size):
    ...
    try:
        ...
        for idx_offset, (orig_idx, para) in enumerate(batch_paras):
            if orig_idx in tagged_map:
                ...
                if "{{" in new_text and new_text != old_text:
                    # 尝试替换
                    if replace_text_in_paragraph(para, para.text, new_text):
                        paragraph_processed += 1

        print(f"    本批次打标成功 {len(tagged_map)} 个")
...
print(f"  标签列表: {tags[:30]}{'...' if len(tags) > 30 else ''}")
